...
function getWhatsAppUrl(settings) {
  return settings?.whatsapp ? `https://wa.me/${String(settings.whatsapp).replace(/\D/g, '')}` : WHATSAPP_URL;
}
...
function AssistancePrompt({ whatsappUrl, onClose }) {
  ...
  return <div className="assistance-modal-backdrop" role="presentation">
    <div className="assistance-modal" role="dialog" aria-modal="true" aria-labelledby="assistance-title">
      ...
      <p>Votre demande a bien été envoyée. Si vous souhaitez échanger directement avec l’équipe, nous pouvons vous rediriger vers WhatsApp.</p>
      <div className="assistance-modal-actions">
        <button className="btn" type="button" onClick={openWhatsApp}>Oui, contacter sur WhatsApp <span>→</span></button>
        <button className="btn outline" type="button" onClick={onClose}>Non, rester ici</button>
      </div>
    </div>
  </div>;
}
...
function RequestForm({ kind }) {
  const [form, setForm] = useState(emptyRequest), [sent, setSent] = useState(false), [error, setError] = useState(''), [loading, setLoading] = useState(false), [showAssistance, setShowAssistance] = useState(false);
  const settings = useChurchSettings();
  const isPrayer = kind === 'prayer';
  const whatsappUrl = getWhatsAppUrl(settings);

  const submit = async (event) => {
    event.preventDefault(); setError(''); setSent(false); setLoading(true);
    try {
  ...
  return <>
    <form className="form-card" onSubmit={submit}>
      <div className="form-grid"><label>Nom{isPrayer ? ' (optionnel)' : ''}<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>Téléphone *<input required value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} placeholder="Votre numéro" /></label>{isPrayer && <><label>Email (optionnel)<input type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="Votre email" /></label><label>Sujet de prière *<input required value={form.subject} onChange={(event) => setForm({ ...form, subject: event.target.value })} placeholder="Sujet de votre demande" /></label></>}</div>
...
export function Prayer() { return <><section className="section prayer-page"><div className="container narrow"><SectionTitle eyebrow="PRIÈRE" title="Votre demande de prière" text="Confiez-nous votre sujet de prière. Nous le porterons dans la prière." /><RequestForm kind="prayer" /></div></section></>; }

export function Evangelism() { return <><section className="section evangelism-page"><div className="container two-cols evangelism"><div><SectionTitle eyebrow="ÉVANGÉLISATION" title="JÉSUS PEUT TRANSFORMER VOTRE VIE" text="Découvrez l’Évangile et commencez une relation personnelle avec Jésus-Christ." /><div className="steps">{[['COMMENT RECEVOIR JÉSUS ?', 'Découvrez comment donner votre vie à Christ.'], ['PRIÈRE DU SALUT', 'Faites cette prière avec foi et sincérité.'], ['BESOIN D’ACCOMPAGNEMENT ?', 'Nous sommes là pour vous accompagner.'], ['TÉLÉCHARGER UNE BIBLE', 'Lisez la Parole de Dieu chaque jour.']].map((item, index) => <article key={item[0]}><i>{index + 1}</i><div><h3>{item[0]}</h3><p>{item[1]}</p></div></article>)}</div></div><div className="sunrise-card"><span>« Jésus vous aime »</span><b>Il veut une relation personnelle avec vous.</b><a href={WHATSAPP_URL}>Je veux être contacté →</a></div></div></section></>; }

export function Help() { const [open, setOpen] = useState(0); const questions = ['Comment assister aux cultes ?', 'Comment faire une demande de prière ?', 'Comment rejoindre un ministère ?', 'Comment faire un don ?', 'Je ne trouve pas l’information dont j’ai besoin']; return <><section className="section help-page"><div className="container help-layout"><div><SectionTitle eyebrow="ASSISTANCE & AIDE" title="Nous sommes là pour vous accompagner" text="Besoin d’une information, d’un accompagnement ou d’une aide pratique ?" />{questions.map((question, index) => <div className="faq" key={question}><button type="button" onClick={() => setOpen(open === index ? -1 : index)} aria-expanded={open === index}><b>{question}</b><span>{open === index ? '−' : '+'}</span></button>{open === index && <p>Pour cette question, contactez directement l’église sur WhatsApp afin d’obtenir une réponse adaptée.</p>}</div>)}<div className="section-inline-action"><a className="btn" href={WHATSAPP_URL}>Contacter l’équipe sur WhatsApp <span>→</span></a></div></div><aside className="help-box"><span className="gold-label">BESOIN D’AIDE ?</span><h2>Nous sommes à votre écoute</h2><a href={WHATSAPP_URL}>WhatsApp<br /><b>Discutez avec nous</b></a><Link to="/prayer">Besoin d’une prière ?<br /><b>Demander une prière →</b></Link></aside></div><section className="section soft"><div className="container narrow"><SectionTitle eyebrow="DEMANDE D’ASSISTANCE" title="Décrivez votre besoin" text="Envoyez-nous directement votre demande d’assistance." /><RequestForm kind="help" /></div></section></section></>; }

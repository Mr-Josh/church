...
export function Header() {
  ...
  const isLinkActive = (to) =>
    to.startsWith("#")
      ? location.pathname === "/" && activeSection === to.slice(1)
      : to === "/"
    ...
    <div className="site-header">
      <header className="header">
        <Link
          ...
          aria-label={`Accueil ${settings.church_name || settings.name}`}
        >
        ...
        >
          {navigation.map(([to, label]) => (
            <Link
              ...
              onClick={(event) => {
                if (to.startsWith("#")) event.preventDefault();
                handleNavigationClick(to, setOpen);
              }}
...
export function Footer() {
  ...
  const whatsapp = settings.phone
    ? `https://wa.me/${String(settings.phone).replace(/\D/g, "")}`
    : WHATSAPP_URL;
    ...
    <footer className="footer">
      <div className="footer-main">
        <div className="footer-brand-group">
          ...
          <p>
            Un ministère envoyé sur le terrain pour annoncer l’Évangile,
            restaurer les vies et servir les communautés.
          </p>
        </div>
        <div className="footer-group">
          ...
          <button
            ...
            onClick={() => goHomeSection("a-propos")}
          >
            À propos
          </button>
          <button
            type="button"
            className="footer-anchor-link"
            onClick={() => goHomeSection("ministeres")}
          >
            Domaines de service
          </button>
          <button
        ...
        <div className="footer-group">
          <h4>Actions</h4>
          <Link to="/prayer" onClick={scrollToPageTop}>
            Demande de prière
          </Link>
          <Link to="/donate" onClick={scrollToPageTop}>
            Soutenir la mission
          </Link>
          <Link to="/testimonials" onClick={scrollToPageTop}>
            Témoignages
          </Link>
          <Link to="/help" onClick={scrollToPageTop}>
            Besoin d'accompagnement
          </Link>
        </div>
        <div className="footer-group footer-contact-group">
          <h4>Nous contacter</h4>
          <span>{settings.address || "Mora, Extrême-Nord"}</span>
      ...
      <div className="footer-bottom">
        <span>
          © 2026 {name}. Gospel Break Chain Ministry. Tous droits réservés.
        </span>
      </div>
    </footer>
  );
...
export function CTA({ children, to = "/contact", dark = false }) {
  return (
    <Link
      className={dark ? "btn outline" : "btn"}
      onClick={scrollToPageTop}
      to={to}
    >
      {children} <span>→</span>

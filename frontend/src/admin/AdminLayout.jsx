...
export default function AdminLayout({ children, counts = {}, title, description, actions, active }) {
  ...
  const logout = async () => {
    try { await churchApi.logout(); } finally { navigate('/admin/login', { replace: true }); }
  };
  ...
  return (
    <div className="admin-shell">
      ...
      <aside className={`admin-sidebar ${mobileOpen ? 'is-open' : ''}`}>
        ...
        <nav aria-label="Administration">
          {navigation.map(([icon, label, to, key]) => {
            ...
            return (
              <Link className={`sidebar-link ${isActive ? 'active' : ''}`} key={to} to={to}>
                <i aria-hidden="true">{icon}</i><span>{label}</span>{badge > 0 && <b>{badge}</b>}
              </Link>
            );
          })}
          <div className="sidebar-link disabled" aria-disabled="true" title="Le module de paiement sera connecté séparément">
            <i aria-hidden="true">▣</i><span>Dons & Offrandes</span><em>À venir</em>
          </div>
        </nav>
      ...
      <main className="admin-main">
        {(title || actions) && <header className="admin-header">
          <div><p className="dashboard-eyebrow">ADMINISTRATION</p>{title && <h1>{title}</h1>}{description && <p>{description}</p>}</div>
          <div className="admin-header-actions">{actions || <Link to="/admin" className="btn outline">← Dashboard</Link>}</div>
        </header>}
        {children}
      </main>
    </div>

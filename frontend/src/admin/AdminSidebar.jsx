...
const navigation = [
  ...
  ["⛁", "Dons & Offrandes", "/admin/donations", "donations"],
...
export default function AdminSidebar({ open = false, onClose }) {
  ...
  const logout = async () => {
    try { await churchApi.logout(); }
    finally { navigate("/admin/login", { replace: true }); onClose?.(); }
  };
  return (
    <>
      ...
      <aside className={`admin-sidebar ${open ? "is-open" : ""}`}>
        ...
        <nav aria-label="Navigation de l’administration">
          {navigation.map(([icon,label,to,key,badgeLabel]) => !to ? <div className="sidebar-link disabled" key={label} aria-disabled="true"><i>{icon}</i><span>{label}</span><em>{badgeLabel}</em></div> : <Link key={label} className={`sidebar-link ${isActive(to) ? "active" : ""}`} to={to} onClick={onClose} aria-current={isActive(to) ? "page" : undefined}><i>{icon}</i><span>{label}</span>{key && <SidebarBadge resource={key} />}</Link>)}
        </nav>

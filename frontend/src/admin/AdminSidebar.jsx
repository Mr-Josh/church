...
const navigation = [
  ...
  ['▣', 'Dons & Offrandes', null, null, 'À venir'],
  ...
  return (
    <>
      ...
      <aside className={`admin-sidebar ${open ? 'is-open' : ''}`}>
        <Link to="/" className="admin-brand" onClick={onClose}>
        ...
        <nav aria-label="Navigation de l’administration">
          {navigation.map(([icon, label, to, key, badgeLabel]) => {
            if (!to) {
              return <div className="sidebar-link disabled" key={label} aria-disabled="true"><i>{icon}</i><span>{label}</span><em>{badgeLabel}</em></div>;
            }
            return (
              <Link
                key={label}
...
function SidebarBadge({ resource }) {
  return <span className="sidebar-badge-slot" data-resource={resource} aria-hidden="true" />;
}

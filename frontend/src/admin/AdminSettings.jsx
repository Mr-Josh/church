...
const fields = [
  ['church_name', 'Nom du ministère'],
  ['slogan', 'Slogan'],
  ['address', 'Zone / adresse de contact'],
  ['phone', 'Téléphone'],
  ['whatsapp', 'WhatsApp'],
  ['email', 'Email'],
  ['pastor_name', 'Nom du pasteur'],
  ['pastor_title', 'Fonction du pasteur'],
...
export default function AdminSettings() {
  ...
  useEffect(() => {
    churchApi.admin.churchSettings().then((response) => setForm(response.data || {})).catch((e) => setError(e.message || 'Impossible de charger les informations.')).finally(() => setLoading(false));
  }, []);

  const submit = async (event) => {
    event.preventDefault(); setSaved(false); setError('');
    try { await churchApi.admin.updateChurchSettings(form); setSaved(true); }
    catch (e) { setError(e.message || 'Impossible d’enregistrer les informations.'); }
  };

  if (loading) return <section className="admin-panel"><p>Chargement...</p></section>;
  return <section className="admin-panel"><form className="form-grid" onSubmit={submit}>
    ...
    <label>Biographie / vision du pasteur<textarea value={form.pastor_bio || ''} onChange={(e) => setForm({ ...form, pastor_bio: e.target.value })} /></label>

...
export default function AdminSettings(){
  const navigate=useNavigate(); const [form,setForm]=useState({}); const [loading,setLoading]=useState(true); const [saved,setSaved]=useState(false); const [error,setError]=useState('');
  useEffect(()=>{churchApi.admin.churchSettings().then(r=>setForm(r.data||{})).catch(e=>{setError(e.message);navigate('/admin/login')}).finally(()=>setLoading(false))},[navigate]);
  const submit=async e=>{e.preventDefault();setSaved(false);try{await churchApi.admin.updateChurchSettings(form);setSaved(true)}catch(e){setError(e.message)}};
  if(loading)return <main className="admin-main"><p>Chargement...</p></main>;

...
export default function AdminContentPage() {
  return <section className="content-hub-grid">{sections.map(([title, description, to, icon]) => <Link className="content-hub-card" to={to} key={title}><span>{icon}</span><div><h2>{title}</h2><p>{description}</p></div><b>→</b></Link>)}</section>;
}

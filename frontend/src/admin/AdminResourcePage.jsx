...
export default function AdminResourcePage({ resource }) {
  const navigate = useNavigate(); const config = resourceConfigs[resource];
  ...
  const load = async()=>{setLoading(true);try{const r=await churchApi.admin.list(resource);setItems(r.data||[]);}catch(e){setError(e.message);if(e.message.toLowerCase().includes('auth'))navigate('/admin/login');}finally{setLoading(false);}};
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><Link to="/" className="admin-brand">Gospel Break Chain Ministry</Link><nav><Link to="/admin">Dashboard</Link>{Object.entries(resourceConfigs).map(([k,v])=><Link key={k} className={k===resource?'active':''} to={`/admin/${k}`}>{v.title}</Link>)}<Link to="/admin/settings">Informations de l’église</Link></nav><button className="admin-logout" onClick={async()=>{await churchApi.logout();navigate('/admin/login')}}>Déconnexion</button></aside>
  <main className="admin-main"><header className="admin-header"><div><p className="eyebrow">ADMINISTRATION</p><h1>{config.title}</h1></div><Link to="/admin" className="btn outline">← Dashboard</Link></header>
  <section className="admin-panel admin-form-panel"><h2>{editing?'Modifier':'Ajouter'} {config.title.toLowerCase()}</h2><form className="form-grid" onSubmit={submit}>{config.fields.map(([name,label,type])=><label key={name}>{label}{type==='textarea'?<textarea value={form[name]} onChange={e=>setForm({...form,[name]:e.target.value})}/>:type==='select'?<select value={form[name]} onChange={e=>setForm({...form,[name]:e.target.value})}><option value="published">Publié</option><option value="draft">Brouillon</option></select>:<input type={type} value={form[name]} onChange={e=>setForm({...form,[name]:e.target.value})} required={['name','title','day'].includes(name)}/>}</label>)}<div className="form-actions"><button className="btn" disabled={saving}>{saving?'Enregistrement...':editing?'Enregistrer les modifications':'Ajouter'}</button>{editing&&<button type="button" className="btn outline" onClick={()=>{setEditing(null);setForm(emptyFor(config.fields))}}>Annuler</button>}</div></form>{error&&<div className="form-error">{error}</div>}</section>
  <section className="admin-panel"><div className="admin-list-head"><h2>Éléments existants</h2><span>{items.length}</span></div>{loading?<p>Chargement...</p>:items.length===0?<p>Aucun élément pour le moment.</p>:<div className="admin-table">{items.map(item=><article key={item.id}><div><strong>{item.name||item.title}</strong><small>{item.description||item.day||item.location||'—'}</small></div><span className={`status ${item.status}`}>{item.status}</span><div className="row-actions"><button onClick={()=>edit(item)}>Modifier</button><button onClick={()=>remove(item.id)}>Supprimer</button></div></article>)}</div>}</section></main></div>;
}

...
if (preg_match('#^/api/donations/confirm/(\d+)$#', $path, $match) && $method === 'POST') {
    ...
    jsonResponse(['message' => 'Donation status updated.', 'status' => $status, 'transaction_id' => $transactionId]);
}

if ($path === '/api/donations/initiate' && $method === 'POST') {
    $pay = require __DIR__ . '/../config/payment.php';
    $data = requestBody();
    if (empty($data['phone']) || empty($data['amount'])) jsonResponse(['message' => 'Le numéro de téléphone et le montant sont requis.'], 422);
    $amount  = (float) $data['amount'];
    $phone   = (string) $data['phone'];
    $name    = !empty($data['name']) ? (string) $data['name'] : 'Donateur anonyme';
    if ($amount <= 0) jsonResponse(['message' => 'Le montant doit être supérieur à 0.'], 422);
    $reference = 'DON-' . strtoupper(bin2hex(random_bytes(6)));
    $stmt = $db->prepare('INSERT INTO donations (reference, name, phone, amount, type, payment_method, status) VALUES (?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([$reference, $name, $phone, $amount, 'Don', 'Genius Pay', 'pending']);
    $donationId = (int) $db->lastInsertId();
    $appUrl     = $config['app_url'] ?? 'http://localhost:8000';
    $frontUrl   = getenv('FRONTEND_URL') ?: 'http://localhost:5174';
    $payload    = json_encode(['amount' => (int) round($amount), 'currency' => $pay['currency'], 'description' => 'Donation – ' . $name, 'customer_name' => $name, 'customer_phone'=> $phone, 'reference' => $reference, 'success_url' => $frontUrl . '/donate?status=success&ref=' . urlencode($reference), 'error_url' => $frontUrl . '/donate?status=cancelled', 'webhook_url' => $appUrl . '/api/donations/webhook']);
    $ch = curl_init($pay['base_url']);
    curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER => true, CURLOPT_POST => true, CURLOPT_POSTFIELDS => $payload, CURLOPT_HTTPHEADER => ['Content-Type: application/json', 'Accept: application/json', 'Authorization: Bearer ' . $pay['api_key']], CURLOPT_TIMEOUT => 15, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_SSL_VERIFYHOST => 0]);
    ...
    $checkoutUrl = $result['data']['checkout_url'] ?? $result['data']['payment_url'] ?? $result['data']['redirect_url'] ?? $result['data']['url'] ?? $result['checkout_url'] ?? $result['payment_url'] ?? $result['redirect_url'] ?? $result['url'] ?? null;
    if (str_contains($pay['api_key'], 'xxxxxxxx')) {
        $stmt = $db->prepare('UPDATE donations SET status = ?, transaction_id = ? WHERE id = ?');
        $stmt->execute(['success', 'TXN-GENIUS-SIM-' . strtoupper(bin2hex(random_bytes(4))), $donationId]);
        jsonResponse(['donation_id' => $donationId, 'reference' => $reference, 'checkout_url' => $frontUrl . '/donate?status=success&ref=' . urlencode($reference), 'is_simulation' => true], 201);
    }
    if ($curlErr) jsonResponse(['message' => 'Erreur de connexion cURL : ' . $curlErr], 502);
    ...
    if (!empty($result['transaction_id'])) { $stmt = $db->prepare('UPDATE donations SET transaction_id = ? WHERE id = ?'); $stmt->execute([$result['transaction_id'], $donationId]); }
    jsonResponse(['donation_id' => $donationId, 'reference' => $reference, 'checkout_url' => $checkoutUrl], 201);
}
...
if ($path === '/api/donations/status' && $method === 'GET') {
    ...
    $stmt->execute([$ref]); $donation = $stmt->fetch();

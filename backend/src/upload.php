...
function handleAdminUpload(PDO $db, array $config): never
{
    ...
    if ($fileSize <= 0 || $fileSize > $maxBytes) jsonResponse(['message' => 'Fichier trop volumineux (max ' . round($maxBytes / 1024 / 1024, 1) . ' Mo).'], 422);

    $tmp = (string) ($file['tmp_name'] ?? '');
    if ($tmp === '' || !is_uploaded_file($tmp)) jsonResponse(['message' => 'Fichier temporaire invalide.'], 422);

    $finfo = new finfo(FILEINFO_MIME_TYPE);
    $mime = $finfo->file($tmp) ?: '';
    if (!in_array($mime, ['image/jpeg', 'image/png', 'image/webp'], true)) jsonResponse(['message' => 'Format non supporté. Utilisez JPG, PNG ou WebP.'], 422);

    $extension = strtolower(pathinfo((string) ($file['name'] ?? ''), PATHINFO_EXTENSION));
    if (!in_array($extension, ['jpeg', 'jpg', 'png', 'webp'], true)) jsonResponse(['message' => 'Extension de fichier non autorisée.'], 422);

    $dimensions = @getimagesize($tmp);
    if (!$dimensions || empty($dimensions[0]) || empty($dimensions[1])) jsonResponse(['message' => 'Le fichier ne contient pas une image valide.'], 422);
    $sourceWidth = (int) $dimensions[0];
    $sourceHeight = (int) $dimensions[1];
    if ($sourceWidth < 1 || $sourceHeight < 1 || $sourceWidth > 12000 || $sourceHeight > 12000) jsonResponse(['message' => 'Dimensions d’image non prises en charge.'], 422);

    $requiredFunctions = [
        'imagewebp',
        $mime === 'image/jpeg' ? 'imagecreatefromjpeg' : ($mime === 'image/png' ? 'imagecreatefrompng' : 'imagecreatefromwebp'),
    ];
    foreach ($requiredFunctions as $function) {
        if (!function_exists($function)) jsonResponse(['message' => 'Le serveur PHP doit disposer de GD avec le support WebP pour traiter les images.'], 503);
    }

    $kind = (string) ($_POST['kind'] ?? 'gallery');
    if (!in_array($kind, ['cover', 'gallery'], true)) jsonResponse(['message' => 'Type de média invalide.'], 422);
    ...
    if (count($safeParts) < 3 || $safeParts[0] !== 'events' || !preg_match('/^\d{4}$/', $safeParts[1])) jsonResponse(['message' => 'Dossier média invalide.'], 422);
    ...
    try {
        ...
        if ($kind === 'cover') {
            generateCoverVariants($source, $dir);
            ...
            $result = generateGalleryVariant($source, $dir, $config, $fileSize, $mime, $sourceWidth, $sourceHeight);
        }
    } catch (Throwable $e) {
        @imagedestroy($source);
...
function generateCoverVariants($source, string $eventDir): void
{
...
function generateGalleryVariant($source, string $eventDir, array $config, int $fileSize, string $mime, int $sourceWidth, int $sourceHeight): array
{
    ...
    $publicRoot = str_replace(DIRECTORY_SEPARATOR, '/', realpath(__DIR__ . '/../public') ?: (__DIR__ . '/../public'));
    $fullAbsolute = str_replace(DIRECTORY_SEPARATOR, '/', $fullPath);
    $thumbAbsolute = str_replace(DIRECTORY_SEPARATOR, '/', $thumbPath);
    $relativeFull = ltrim(str_replace($publicRoot, '', $fullAbsolute), '/');
    $relativeThumb = ltrim(str_replace($publicRoot, '', $thumbAbsolute), '/');
    $path = '/' . $relativeFull;
    $thumb = '/' . $relativeThumb;
    ...
    return [
        ...
        'generated' => [
            ...
            'thumbnailSize' => is_file($thumbPath) ? filesize($thumbPath) : 0,
        ],
    ];
}
